...
void main(int argc, char **argv) {
  ...
  const char usage[80] = "Usage:  merge -i [...] -o [...]";
  ...
  for(i = 1; i < argc; i++)
    if(**(argv+i) == '-')
      switch(argv[i][1])
        ...
        case 'i':
	  if (I) {
	    cout << "You have already specified the similar nodes!" << endl;
	    exit(1);
	  }
	  I = i;
          break;
        case 'o':
	  if (O) {
	    cout << "You have already specified the supernodes!" << endl;
	    exit(1);
	  }
	  O = i;
          break;
  ...
  if (!I) {
    cout << "You have failed to specify the similar nodes!" << endl;
    exit(1);
  }
  else if (!O) {
    cout << "You have failed to specify the supernodes!" << endl;
    exit(1);
  }
  ...
  for(i = 0; i < in; i++) {
    cout << "Similar node " << input_node[i] << " has entities ";
  ...
  for(i = 0; i < out; i++) {
    cout << "Supernode " << output_node[i] << " has entities ";
    for(j = 0; j < output_entity_count[i]; j++)
      cout << output_entity_set[i][j] << " ";
    cout << endl;
  }
  cout << "Verify that these entity sets are equal." << endl << endl;
  ...
  for(i = 0; i < out; i++) {
	...
	for(n = 0; n < input_entity_count[m] && !match; n++)
      ...
      if (match) {
	cout << "Supernode " << output_node[i];
	cout << " is subset of similar node " << input_node[m] << endl;
    ...
    cout << "Combine the visibility regions of these similar nodes."<<endl<<endl;
  }
}
